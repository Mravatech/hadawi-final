  ...
  Widget build(BuildContext context) {
    return BlocConsumer<OccasionsListCubit, OccasionsListStates>(
      ...
      builder: (context, state) {
        if (state is GetClosedOccasionListLoadingState) {
          return Center(
        ...
        final occasions = OccasionsListCubit.get(context).closedOccasionsList;
        
        if (state is GetClosedOccasionListSuccessState && occasions.isNotEmpty) {
          return RefreshIndicator(
            color: Color(0xFF8B7BA8),
            onRefresh: () async {
              await OccasionsListCubit.get(context).getClosedOccasionsList();
            },
